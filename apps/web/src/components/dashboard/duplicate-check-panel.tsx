...
import type { DuplicateCheckScope } from "@ai-shopify/shared";
            ...
            return (
              <div key={job.id} className="rounded-lg border p-3 text-sm">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ...
                  {job.status === "COMPLETED" &&
                    (job.deletedCount > 0
                      ? `Removed ${job.deletedCount} duplicate${job.deletedCount === 1 ? "" : "s"} — ${job.contentDuplicatesRemoved} by repeated content, ${job.reviewerDuplicatesRemoved} by reviewer reused across products (scanned ${job.scannedCount})`
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Runs in the background — finds reviews with repeated content or the same reviewer
              reused across more than one product, and deletes the duplicates (keeping the
              earliest one).
            </DialogDescription>
          </DialogHeader>
